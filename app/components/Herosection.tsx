...
//states
  ...
  const handleWatchTrailer = () => {
    axios.get(`${BASE_URL}/movie/${currentMovie.id}/videos`, {
  ...
  return (
    <div className="relative w-full h-[600px] overflow-hidden rounded-xl bg-black">
      ...
      <div className="absolute inset-0 flex flex-col justify-end p-10 bg-gradient-to-t from-black/80 to-transparent">
        <h1 className="text-4xl font-bold text-white mb-2">{currentMovie?.title}</h1>
        <p className="text-yellow-400 font-bold mb-4">★ {currentMovie?.vote_average?.toFixed(1)}</p>
        <p className="text-gray-200 max-w-xl line-clamp-3 mb-6">{currentMovie?.overview}</p>
      ...
      <button onClick={prevSlide} className="absolute left-4 top-1/2 -translate-y-1/2 text-white text-4xl p-2">❮</button>
      <button onClick={nextSlide} className="absolute right-4 top-1/2 -translate-y-1/2 text-white text-4xl p-2">❯</button>
